      ...
      <div class="wrapper">
        ...
        <div class="active_page_content">
          ...
          <div class="active_section_box">
            ...
            <div class="section_box_wrapper">
              <div class="active_time">
                活动期间新会员首次存款时可申请一次电竞首存优惠，申请成功的用户可享受单笔存款最高35%的存款优惠。
              </div>
              <table align="center"  width="100%">
                <tbody v-if="initInfo&&initInfo.active_id">
                  ...
                  <!-- <tr>
                    ...
                    <td rowspan="4">（本金+红利）x{{initInfo&&initInfo.active_bet_times}} 电竞</td>
                  </tr> -->
                  <tr>
                    <td rowspan="4">
                    ...
                    <td rowspan="4">≥{{initInfo&&initInfo.active_detail[1].low_money}}</td>
                    <td rowspan="4" class="yellow">{{initInfo&&initInfo.active_detail[1].percent}}</td>
                    <td rowspan="4">{{initInfo&&initInfo.active_detail[1].max_bonus}}</td>
                    <td rowspan="4">（本金+红利）x{{initInfo&&initInfo.active_bet_times}} 电竞</td>
                  </tr>
                </tbody>
          ...
          <div class="active_section_box">
            ...
            <div class="section_box_wrapper">
              <div class="active_desc">
               <p>
                1.符合条件的会员可在点击本优惠活动页面上“立即首存”按钮后，选择“小艾电竞钱包”、“RG电竞钱包”、“IM电竞钱包”其中之一进行首存转账。对应的存款本金+活动奖金在转账成功后将自动添加至用户选择的电竞游戏钱包中。<br>
                2.此活动首笔存款定义为：用户注册后的第一笔存款；若用户在申请前将首笔存款从主钱包转出，则不可申请首存优惠。<br>
                3.申请奖金后需在指定电竞游戏中完成相对应的流水要求，当用户对应电竞钱包余额＜最低投注额且无未结算注单时，系统将自动判定已完成活动规定的有效流水。<br>
                4.此活动不与其它首存活动共享，每个账户仅限申请一次；不可与老用户召回存款活动同一天参与；领取了此项优惠活动的新用户需先完成活动规定的流水后才可申请平台救援金活动。<br>
                5.有效流水仅计算产生输赢结果的注单，电竞赔率＜1.5、走盘、注单取消、对冲、未结算、串关、平半盘、实时兑现等不计算为有效流水。<br>
...
export default {
  ...
  mounted () {
    this.getEgameFirstTransferInit();
    this.selectedObj = this.qianbaoarr[0]
  },
  methods: {
    getEgameFirstTransferInit() {
      this.$http(this.ApiSetting.active.getEgameFirstTransferInit)
      .then(res => {
    ...
    depositMoneyChange () {
      ...
      let obj = {}
      obj = this.selectedObj.code === 'EG' ? this.initInfo.active_detail[0] : this.initInfo.active_detail[1]
...
<style scoped>
.ag_active{
  background: url("../../../assets/active/egameshoucun-banner.jpg") no-repeat center top;
  position: relative;
}
...
.sport_rescue_dialog .wallet_box {
  margin: 0 15px 15px 0;
}
.sport_rescue_dialog .wallet_box:nth-child(2n){
  margin-right: 0;
}
