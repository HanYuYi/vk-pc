<template>
  <section class="vkLucky-draw">
    <div
        ...
        <p class="prize-index">{{ item.goods_name }}</p>
    </div>
    <div class="btn">
      <div class="bg-img" @click="$emit('onStart')"></div>
      <b>每次抽奖消耗<span style="color: red;">1</span>张抽奖卷</b>
    </div>
  </section>
</template>

<script>
import Cubic from "../../../../../utils/cubic"
export default {
  name: 'vkLuckyDraw',
    props: {
      prizesList: {
...
.vkLucky-draw{
  ...
  grid-gap: 60px; /* grid-column-gap 和 grid-row-gap的简写 */
  grid-auto-flow: row;
  padding: 58px 20px;
  position: relative;
  .prize-item{
    ...
    height: 158px;
    color: #101010;
    background-image: url("../../../../../assets/active/egame/msi2023/raffle-gift-bg.png");
  ...
  .speed-chose{
    background-image: url("../../../../../assets/active/egame/msi2023/raffle-gift-active-bg.png");;
    background-repeat: no-repeat, no-repeat;
    background-size: 100% 100%;
  ...
  .btn{
    ...
    .bg-img{
      width: inherit;
      height: inherit;
      background-image: url('../../../../../assets/active/egame/msi2023/cheers-raffle-btn.png');
      background-repeat: no-repeat, no-repeat;
