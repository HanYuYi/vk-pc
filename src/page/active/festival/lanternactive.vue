         ...
         <div class="active_page_content">
           ...
           <div class="active_section_box">
             ...
             <div class="section_box_wrapper">
               ...
               1. 用户在活动期间满足最低有效投注金额要求，并在活动页面对应红包处点击报名。报名成功的用户即可在元宵节当天规定时间内参与瓜分66万元团圆红包，红包金额最低8元起。<br>
               2. 活动期间累计有效投注额越大，则所领取的红包金额越大。
               </div>
               <div class="active_desc clearfix margin-t-20">
           ...
           <div class="active_section_box">
             ...
             <div class="section_box_wrapper">
               <div class="active_desc margin20">
                ...
                3. 娱乐玩法：AG真人，欧博真人，PG游戏<br>
                  &nbsp;&nbsp;&nbsp;&nbsp;棋牌玩法：乐游棋牌，幸运棋牌，凯旋棋牌，博乐棋牌  <br>
                  &nbsp;&nbsp;&nbsp;&nbsp;体育玩法：威客体育，平博Sports<br>
                  &nbsp;&nbsp;&nbsp;&nbsp;电竞玩法：威客电竞，RG电竞，IM电竞<br>
                4. 有效投注额仅计算在对应游戏中产生输赢结果的注单，电竞赔率＜1.45、香港盘赔率<0.75、欧洲盘<1.75、美洲盘≤-133、马来盘≤0.75且>0、走盘、注单取消、对冲、未结算、自走棋等注单不计算为有效投注。<br>
...
import { formatNumberSplit } from '@/utils/util'
   ...
   methods: {
     ...
     doubleHandel(item,type) {
       ...
       if(item.btn_status==2) {
         this.$http(this.ApiSetting.active.joinLanternFestival, {type:type})
         .then(res => {
         ...
         this.$http(this.ApiSetting.active.getLanternFestivalRewards,{type:type})
         .then(res => {
           if(res.status == 1 && res.data) {
             if(res.status == 1) {
                this.$message({
     ...
     getBase() {
       this.baseLoad = true
       this.$http(this.ApiSetting.active.getLanternFestivalInit)
       .then(res => {
         this.baseLoad = false
         if(res.status == 1 && res.data) {
 ...
 .doubleeleven{
   background: url("../../../assets/active/lanternactive/banner.jpg") no-repeat center top;
   .brick {
      margin-top: 30px;
      position: relative;
      overflow: hidden;
     li {
       ...
       background:url("../../../assets/active/lanternactive/bor.png") no-repeat;
       ...
       &:nth-child(1) {
         .titimg {
           background: url("../../../assets/active/lanternactive/1.png") no-repeat;
         }
       }
       &:nth-child(2) {
         .titimg {
           background: url("../../../assets/active/lanternactive/2.png") no-repeat;
         }
       }
       &:nth-child(3) {
         .titimg {
           background: url("../../../assets/active/lanternactive/3.png") no-repeat;
         }
       }
       &:nth-child(4) {
         .titimg {
           background: url("../../../assets/active/lanternactive/4.png") no-repeat;
         }
       }
       .titimg {
         position: absolute;
      ...
      .apply_btn {
        ...
        &.apply_end {
          ...
          background-color: rgba(80,14,30,0.9);
          color: #c3c3c3;
        }
        &.notice {
          background-image:none;
